            ...
            return Q.step(
                ...
                function() {
                    ...
                    return args.pkg.reduce(function(done, pkg) {
                        return Q.all([done, exports.debianize(pkg, opts)]).get(0);
                    }, undefined);
...
cacheRead = function(pkg, ver, forceBypass) {
...
cacheUnpack = function(pkg, ver, targetPath) {
...
exports.debianize = function(pkg, opts) {
    ...
    return Q.step(
        ...
        function() {
          console.log('debianize completed');
        }
    ).fail(function(err) {
          console.log('debianize failed ' + err);
    });
};

nameToPackageName = {};

exports.getNameToPackageName = function(name) {
  return nameToPackageName[name];
}

makeSourcePackage = function(pkg, ver, opts) {
    var ctx = {};
    ...
    ctx.debianName = ctx.debianNamePrefix + ctx.pkg.toLowerCase();
    ...
    if(ctx.devDependencies && ctx.dependencies) {
      ...
      ctx.npmlink = "@echo 'package has no dependencies'";
    }

    pkgDir = ctx.debianName + '-' + ver;
        ...
        function(exists) {
            ...
            steps.push(
                ...
                function(packageData) {
                    ...
                    ctx.depends = semverToDebian('lophilo-nodejs', nodeVer);
            ...
            return Q.step.apply(null, steps).then(function() {
              return pkgName;
            });
        }
    );
...
filterObjectKeys = function(obj, cb) {
    cb = cb || function() {
        return true;
    };
    var keys = [];
...
dh_make = function(dir, ctx) {
    ...
    var child = CP.exec(cmd, opts, function(err, stdout, stderr) {
        err? d.reject(err +':'+ cmd + '\n' + stdout + stderr) : d.resolve();
    });
    child.stdin.write('\n');
...
tplDebianFile = function(path, ctx) {
...
semverToDebian = function(pkg, ver) {
    if(!ver) return pkg;
